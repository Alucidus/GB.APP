...
**Consolidating survivors.** At any point, a side may **merge its surviving squads into a single combined squad**, pooling their remaining HP up to the normal 8-HP maximum. Three squads at 3, 4, and 1 HP become one 8-HP squad rolling a full 8 Firepower, rather than being picked off individually at 4 or 5 dice each.

This is a genuine last-stand option with a real trade-off: consolidating restores full fighting strength for one final defence, but leaves no reserve squads and no bodies to cover additional pairings — if the combined squad falls, that side has nothing left. It exists so that a battered defender can still mount a credible comeback in the deciding engagement instead of being ground down by attrition they can no longer answer.
