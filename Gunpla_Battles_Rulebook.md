...
| **Smoke Grenade** | Clears all of the user's own current suppression | 2 | Grenade (declared same round) | Suppression doesn't clear |
| **Grenade** | 1 guaranteed casualty on the enemy squad — no roll needed | 2 | Flashbang (declared same round) | **The blast lands on the thrower instead.** Flashed mid-throw, the soldier drops the grenade at their own feet — the intended target takes nothing, and the Grenade's own squad suffers **1 casualty**. |

**cf120 supply loadout:** every Quick Resolve squad starts with 2 Flashbangs, 2 Smoke Grenades and 2 Grenades. Their counter-effects are unchanged. This change applies to the squad's Quick Resolve bag, not individual-soldier specialist equipment on the Detailed Battle Map.
...
**Squad Items scale by effect, not by squad count.** A squad does **not** receive extra Flashbangs, Smoke Grenades, or Grenades for being outnumbered — each squad has its own 2 Flashbangs / 2 Smoke Grenades / 2 Grenades bag, with no extra charges granted for being outnumbered. Because the fights resolve one at a time, items only ever affect the enemy squad currently engaged; there is no splash onto a squad waiting for its own segment.
...
**Item Charges Only Refill By Resupply (cf120):** Quick Resolve items never refill when a fight ends, another bout starts, a page refreshes or a device reconnects. A squad can restock aboard a Car, Helicopter or Transport Ship after an uninterrupted full own-turn cycle. **Example: board during your turn 1; your turn 2 starts the service cycle; stay aboard for that turn and the intervening enemy turn; refill at the start of your turn 3.** All three supplies return to 2. Boarding during an enemy turn likewise waits for the next own-turn start to begin the cycle. Leaving, transferring to another transport, losing the transport, removal from the roster or destruction of the squad cancels progress immediately. Reboarding starts a fresh wait, even aboard the same transport. Mere proximity or approaching a transport does not qualify.

The online room advances service once per official turn transition and controls Quick Resolve item spending. Offline play retains the manual **Resupplied** button for tabletop tracking. Loading an older battle preserves its remaining item counts; it does not add the extra Smoke/Grenade automatically. Those become available on the next completed resupply (or the explicit offline Resupplied action). Old ambiguous resupply progress is restarted without restoring items. New squads start with the new full loadout.
