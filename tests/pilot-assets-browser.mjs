...
try{
 ...
 for(const body of ['female','male']){
  ...
  for(let face=0;face<4;face++){
   ...
   await page.evaluate(()=>{window.pilotBefore=document.getElementById('pilotCanvas').getContext('2d').getImageData(0,0,1254,1500).data;});
   await choose('eyeColor','#4c91cd');
   const diff=await page.evaluate(()=>{const before=window.pilotBefore,after=document.getElementById('pilotCanvas').getContext('2d').getImageData(0,0,1254,1500).data;let changed=0;const escaped=[];for(let i=0;i<after.length;i+=4){if(before[i+3]<240||after[i+3]<240)continue;if(Math.max(Math.abs(before[i]-after[i]),Math.abs(before[i+1]-after[i+1]),Math.abs(before[i+2]-after[i+2]))<=2)continue;changed++;const x=(i/4)%1254,y=Math.floor(i/4/1254);if(!(y>=720&&y<=850&&((x>=440&&x<=545)||(x>=700&&x<=800)))){if(escaped.length<5)escaped.push({x,y});}}delete window.pilotBefore;return {changed,escaped};});
