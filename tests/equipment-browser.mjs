...
try {
 for(const viewport of [{width:1400,height:1000},{width:844,height:390}]) {
  ...
  for(const side of ['federation','spacenoid']) {
   ...
   if(process.env.EQUIPMENT_SCREENSHOT)await page.screenshot({path:process.env.EQUIPMENT_SCREENSHOT+'-'+side+'-sheet-'+viewport.width+'.png'});
   ...
   assert.equal(await page.locator('#pickT').textContent(),'Swap equipped weapon?');
   assert.equal(await page.evaluate(()=>eqPending.key),'beam-saber-x2');checks+=3;
   if(process.env.EQUIPMENT_SCREENSHOT)await page.screenshot({path:process.env.EQUIPMENT_SCREENSHOT+'-'+side+'-swap-'+viewport.width+'.png'});
   await page.locator('#pickCancel').click();
   assert.equal(await page.evaluate(()=>JSON.stringify([CUR.st.eq,ap,hp,sh,wpn])),preSwap);checks++;
   await page.locator('.hp.eq-arm[title="Equip in Right Arm"]').click();
   await page.keyboard.press('Escape');
   assert.equal(await page.locator('#pick').evaluate(n=>n.classList.contains('on')),false);checks++;
   await page.locator('.hp.eq-arm[title="Equip in Right Arm"]').click();
   await page.evaluate(()=>turn.round++);
   await page.locator('#eqSwapConfirm').click();
   assert.equal(await page.locator('#pick').evaluate(n=>n.classList.contains('on')),true,'changed state requires fresh confirmation');
   assert.equal(await page.evaluate(()=>JSON.stringify([CUR.st.eq,ap,hp,sh,wpn])),preSwap);checks+=2;
   await page.locator('#eqSwapConfirm').click();
   ...
   assert.equal(await page.locator('#pick').evaluate(n=>n.classList.contains('on')),false);
   assert.equal(dialogs.length,0,'equipment uses no browser dialogs');checks+=2;
