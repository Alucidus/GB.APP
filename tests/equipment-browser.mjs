...
try {
 for(const viewport of [{width:1400,height:1000},{width:844,height:390}]) {
  ...
  for(const side of ['federation','spacenoid']) {
   ...
   await page.locator('#pick .eq-card').filter({has:page.locator('b',{hasText:'Beam Saber'})}).getByRole('button',{name:'Equip weapon',exact:true}).click();
   ...
   await page.locator('.aphit[title="Equipped melee — tap to assign another arm"]').click();
  ...
  if(process.env.EQUIPMENT_SCREENSHOT){await page.screenshot({path:process.env.EQUIPMENT_SCREENSHOT+'-'+viewport.width+'.png'});}
  await page.close();
 }
