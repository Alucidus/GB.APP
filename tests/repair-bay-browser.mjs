...
try{
 for(const viewport of [{width:667,height:375},{width:844,height:390},{width:932,height:430},{width:1400,height:1000}]){
  ...
  await page.locator('[data-repair-slot="0"] .repair-slot-select').tap();assert.deepEqual(await page.evaluate(()=>CUR.st.ship.repairSlots),[null,null,null],'slot tap alone never assigns a unit');checks++;
  ...
  assert.match(await page.locator('.repair-base-empty').innerText(),/Tap slot 1 to confirm base territory/);checks++;
  ...
  await page.locator('.repair-base-bay .repair-slot-select').tap();
  ...
  await page.locator('#repairBaseAction').tap();assert.match(await page.locator('#pickS').innerText(),/will return to the base roster and lose its unfinished repair cycle/);checks++;
  ...
  await page.locator('#baseBtn').tap();await page.locator('#repairBaseAction').tap();await page.locator('#pickExtra button').tap();
  ...
  assert.match(await page.locator('.repair-base-empty').innerText(),/No repairable damage remains/);assert.equal(await page.locator('.repair-candidate button').textContent(),'No repairs needed');checks+=2;
  await page.close();
 }
