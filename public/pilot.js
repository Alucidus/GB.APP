...
window.Pilot = (() => {
  ...
  function tiles(label,key,names,images){const field=choices(label,key,names.map((name,i)=>[name,i]));field.classList.add('pilot-tile-field');field.querySelectorAll('button').forEach((b,i)=>{const caption=document.createElement('span');caption.textContent=b.textContent;b.replaceChildren();const img=document.createElement('img');img.src=images[i];img.alt='';img.loading='lazy';b.append(img,caption);b.classList.add('pilot-tile');});return field;}
