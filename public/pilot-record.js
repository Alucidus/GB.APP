...
window.PilotRecord=(()=>{
 ...
 function render(host,p,onShop,onHangar){
  ...
  else rank.append(el('p','Highest rank achieved · Legendary Newtype'));
  ...
  const actions=el('nav',null,'pilot-record-actions');actions.setAttribute('aria-label','Pilot destinations');for(const [name,fn]of[['Unit shop',onShop],['Your hangar',onHangar]]){const b=el('button',name,'btn');b.type='button';b.onclick=fn;actions.append(b);}host.append(actions);
